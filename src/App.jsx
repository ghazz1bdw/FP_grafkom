...
function App() {
  useEffect(() => {
    ...
    glftLoader.load('./assets/panggung.glb', (gltfScene) => {
      loadedModel = gltfScene;
      // console.log(loadedModel);
      gltfScene.scene.traverse((child) => {
        if (child.isMesh) {
          child.castShadow = true;
          child.receiveShadow = true; // Set to true to make the mesh receive shadows
        }
      });
    ...
    dlFolder.add(dl.position, 'y', 1, 100, 0.5);
    ...
    const sl = new THREE.SpotLight(0x00ff00, 1, 100, Math.PI / 4, 0);
    sl.position.set(0, 100, 20);
